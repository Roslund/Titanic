...
class NeuralNet {
    ...
    init(inputSize: Int, hiddenSize: Int, outputSize: Int) {
        weights1 = Matrix.random(rows: inputSize+1, columns: hiddenSize, range: -1.0 ... 1.0)
...
extension NeuralNet {
    func guess(_ input: Matrix) -> Matrix {
        var baiasInput = input.grid
        baiasInput.append(1.0)
        let hidden = sigmoid(Matrix(baiasInput)  <*> weights1)
...
extension NeuralNet {
    func train(input: Matrix, awnser: Matrix) {
        ...
        weights1 += Matrix(baiasInput)′  <*> (hidden_delta)
